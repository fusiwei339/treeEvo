    ...
    Deps.autorun(() => {
        ...
        if (handler.ready()) {
            ...
            var treeGroups = d3.nest()
                ...
                .entries(trees)
            treeGroups = treeGroups.sort((a, b) => a.values.length - b.values.length);

            // var matrixView=d3.select('#matrixView');
            // matrixView.selectAll('*').remove();
            // var canvas = matrixView.append('g');
            var canvas=d3.select('#matrixView');

            new d3.drawMatrix(canvas, treeGroups)
                .draw();
        }
    })
