...
geom = function() {
    ...
    */
    g.path = {
        ...
        arc_to: function(xradius, yradius, arcSweep, endX, endY) {
            var arr = ["A", xradius, yradius, 0, 1, arcSweep, endX, endY]
            this.value += arr.join(' ');
            return this;
        },
    ...
    g.scaling = function(width, height, objects, getter, setter) {
        ...
        var x_min = algo.min(objects, function(n) {
                return getter.call(null, n).x;
            }),
            x_max = algo.max(objects, function(n) {
                return getter.call(null, n).x;
            }),
            y_min = algo.min(objects, function(n) {
                return getter.call(null, n).y;
            }),
            y_max = algo.max(objects, function(n) {
                return getter.call(null, n).y;
            });
